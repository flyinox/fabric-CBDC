...
func (s *SmartContract) Transfer(ctx contractapi.TransactionContextInterface, recipient string, amount int) error {
	...
	// 获取接收方MSP（简化实现，从用户ID推断MSP）
	recipientMSP := s.getUserMSPFromID(recipient)
...
func (s *SmartContract) getBalanceFromPrivateCollection(ctx contractapi.TransactionContextInterface, userID string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to read balance from private collection: %v", err)
	}

	if balanceBytes == nil {
		return 0, nil // 账户不存在，返回0余额
	}
...
func (s *SmartContract) updateBalanceInPrivateCollection(ctx contractapi.TransactionContextInterface, userID string, balance int) error {
	...
	// 获取用户MSP
	userMSP := s.getUserMSPFromID(userID)
...
// 辅助函数：从用户ID推断MSP（简化实现）
func (s *SmartContract) getUserMSPFromID(userID string) string {
	// 简化实现：根据用户ID特征推断MSP
	// 实际应用中可能需要维护用户-MSP映射表
	if strings.Contains(userID, "centralbank") {
		return "CentralBankMSP"
	} else if strings.Contains(userID, "aa") {
		return "aaMSP"
	} else if strings.Contains(userID, "bb") {
		return "bbMSP"
	}
	// 默认返回调用者的MSP
	return "aaMSP" // 可以根据实际情况调整
}
