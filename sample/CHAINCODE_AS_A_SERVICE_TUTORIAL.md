...
The chaincode-as-a-service feature is a very useful and practical way to run 'Smart Contracts'. Traditionally the Fabric Peer has taken on the role of orchestrating the complete lifecycle of the chaincode. It required access to the Docker Daemon to create images, and start containers. The Go chaincode framework was explicitly known to the peer including how it should be built and started.
...
The `test-network` and Go chaincode have been updated to support running chaincode-as-a-service. The commands below assume that you've got the latest fabric-samples cloned, along with the latest Fabric docker images.
...
- Build a docker image of the contract. The `/asset-transfer-basic/chaincode-go` has been updated with Dockerfile
...
```bash
./network.sh deployCCAAS  -ccn basic -ccp ../asset-transfer-basic/chaincode-go
```
...
## How to configure Go language

Go chaincode can work in the '-as-a-service' mode. Note that the approach here is based on the very latest libraries.
When starting the image you can also specify any of the TLS options or additional logging options for the Go chaincode libraries.

### Go

With the latest Go Chaincode libraries, there are minimal code changes to make. The '-as-a-service' mode will be used if the environment variable `CHAINCODE_SERVER_ADDRESS` is set.

A sample docker run command could be as follows. The two key variables that are needed are the `CHAINCODE_SERVER_ADDRESS` and `CORE_CHAINCODE_ID_NAME`

```bash
    docker run --rm -d --name peer0org1_basic_ccaas  \
                  --network fabric_test \
...
```bash
./network.sh deployCCAAS  -ccn basic -ccp ../asset-transfer-basic/chaincode-go -ccaasdocker false
#....
Not building docker image; this the command we would have run
docker build -f ../asset-transfer-basic/chaincode-go/Dockerfile -t basic_ccaas_image:latest --build-arg CC_SERVER_PORT=9999 ../asset-transfer-basic/chaincode-go
#....
Not starting docker containers; these are the commands we would have run
    docker run --rm -d --name peer0org1_basic_ccaas                    --network fabric_test                   -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999                   -e CHAINCODE_ID=basic_1.0:59dcd73a14e2db8eab7f7683343ce27ac242b93b4e8075605a460d63a0438405 -e CORE_CHAINCODE_ID_NAME=basic_1.0:59dcd73a14e2db8eab7f7683343ce27ac242b93b4e8075605a460d63a0438405                     basic_ccaas_image:latest
```
...
To manually build the docker image for the `asset-transfer-basic/chaincode-go`
...
Go chaincode for example, could be started like this

```bash
 docker run --rm -it --name peer0org1_basic_ccaas --network fabric_test -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:9999 -e CHAINCODE_ID=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 -e CORE_CHAINCODE_ID_NAME=basic_1.0:7c7dff5cdc43c77ccea028c422b3348c3c1fb5a26ace0077cf3cc627bd355ef0 basic_ccaas_image:latest
```

For Go chaincode please note:
