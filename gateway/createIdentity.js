...
// 自动查找signcerts目录下的证书文件
const signcertsDir = path.resolve(__dirname, `../organizations/peerOrganizations/${centralBankOrg.domain}/users/Admin@${centralBankOrg.domain}/msp/signcerts`);
let certFile = null;
...
if (!certFile) {
  console.error('signcerts目录下未找到证书文件');
  process.exit(1);
}
const certPath = path.join(signcertsDir, certFile);

// 自动查找keystore目录下的私钥文件
const keystoreDir = path.resolve(__dirname, `../organizations/peerOrganizations/${centralBankOrg.domain}/users/Admin@${centralBankOrg.domain}/msp/keystore`);
let keyFile = null;
...
if (!keyFile) {
  console.error('keystore目录下未找到私钥文件');
  process.exit(1);
}
...
const identityPath = path.join(walletPath, 'admin.id');
fs.writeFileSync(identityPath, JSON.stringify(identity, null, 2));

console.log('身份文件创建成功:', identityPath);
